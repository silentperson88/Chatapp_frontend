...
function Sidenav({ color, brand, brandName, routes, ...rest }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRole(token.role);
      await dispatchThunk(GetAllGroups());
    })();
  }, [Session.userToken]);
  ...
  const handleRoutingAsPermission = (key, route, accessibleTo, name, icon) => {
    ...
    if (accessibleTo === "admin" && role === "admin") {
      return (
        <NavLink key={key} to={route}>
          <SidenavCollapse name={name} icon={icon} active={key === collapseName} />
        </NavLink>
      ...
      <List sx={{ marginX: 1, marginTop: 0.5 }}>
        ...
        {groups &&
          ...
          groups.groupList.map((group) => (
            <MDBox key={group.id}>
              <NavLink key={group.name} to={`/group/${group[Constants.MONGOOSE_ID]}`}>
                <SidenavCollapse
                  name={group.name}
                  icon="groups"
                  active={group.name === collapseName}
                  parent=""
                />
              </NavLink>
            </MDBox>
